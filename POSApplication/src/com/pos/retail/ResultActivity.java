		...
		if (data != null) {
			...
			if ("paymentResult".equals(host)) {
				...
				if ("Cancel".equalsIgnoreCase(type)) {
					...
					TransactionSaveOnWeb transaction = new TransactionSaveOnWeb(this,PayPalClass.TotalTax,PayPalClass.TotalDiscount,PayPalClass.payableMoney,PayPalClass.paymentMethod,PayPalClass.voucherName,PayPalClass.voucherId,3,PayPalClass.paymenttype,PayPalClass.strDiscount, PayPalClass.finalAmount, PayPalClass.subTotalAmount, 0, String.valueOf(System.currentTimeMillis()/1000), false, "0", PayPalClass.finalAmount);
																
				    transaction.new SaveTransaction().execute(PayPalClass.productList);
