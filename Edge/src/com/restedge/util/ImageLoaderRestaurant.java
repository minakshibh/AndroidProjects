...
public class ImageLoaderRestaurant {
	private Map<String, Bitmap> cache=Collections.synchronizedMap(
            new LinkedHashMap<String, Bitmap>(20,1.5f,true));

	   
//	private Map<ImageView, String> cache = Collections
//            .synchronizedMap(new WeakHashMap<ImageView, String>());
	    ...
	    public ImageLoaderRestaurant(Context context){
    	...
    	RestaurantDatabase handler=new RestaurantDatabase(mcontext);
    	
    	
    	 int id=Integer.parseInt(url);
    	Place place=handler.getPlace(id);
    	
    	byte image[]=place.getPlaceImage();
    	 handler=new RestaurantDatabase(mcontext);
    	 
    	if(image==null)
    	{
    		byte[] imagebyte=null;
    		 imagebyte=getDecodeUrl(place.getImageUrl());
    		place.setPlaceImage(imagebyte);
    		
    		
    		handler.updatePlace(place.getId(), place);
    		b=decodeFile(imagebyte);
    		return b;
    	}
    	else
    	{
...
//	         
	         ...
	         System.out.println("scale "+sampleSize);
...
//	    public Bitmap scaleMethod(InputStream is)
//	    {
//	    	
//	    	Bitmap bitmap=null;
//	    	 BitmapFactory.Options o = new BitmapFactory.Options();
